...
// ✅ FIX: Funzione toJSON che gestisce Date correttamente
function toJSON(obj) {
  var parts = [];
  for (var key in obj) {
    if (obj.hasOwnProperty(key)) {
      ...
      if (typeof v === 'string') {
        ...
        // ✅ Converti Date in timestamp numerico
        jsonValue = '""';
      }
...
// ✅ FIX: updateStatus senza usare new Date() direttamente
function updateStatus(progress, status, error) {
  if (!statusPath) return;
  try {
    var statusFile = new File(statusPath);
    // Usa timestamp numerico invece di Date object
...
try {
  ...
  // ✅ Trova il file job_data più recente
  ...
  if (files && files.length > 0) {
    ...
    log('⚠️ Fallback a job_data.json');
  }

  var job = parseJSONFile(jsonPath);
  if (!job.job_id) {
    job.job_id = 'alifind_' + new Date().getTime();
  }
  ...
  if (!job.products || job.products.length < 1) {
    throw new Error('job.products deve avere almeno 1 prodotto');
  }

  // Chiudi progetto esistente
  if (app.project) {
    try {
  ...
  // ✅ OUTRO - DISABILITA TUTTI POI ABILITA UNO
  ...
  try {
    ...
    log('⚠️ Layer ' + outroLayerName + ' non trovato!');
  }
  ...
  if (outro) {
    ...
    log('📍 OUTRO posizionato: startTime=' + outroStart.toFixed(2) + 's');
  }

  // PRODOTTI
  var N = job.products.length;
  var PRODUCT_DURATION = 3.0;
  log('Timing prodotti: N=' + N + ', durata=' + PRODUCT_DURATION + 's');
  ...
  for (var i = 0; i < N; i++) {
    ...
    log('Layer startTime=' + start.toFixed(2) + 's');
  ...
  if (job.hero_lines && job.hero_lines.length > 0) {
    ...
    try {
      ...
      if (!heroTextLayer.source || !(heroTextLayer.source instanceof CompItem)) {
        throw new Error('HERO_TEXT non precomp');
      }
      ...
      if (!heroLineLayer.source || !(heroLineLayer.source instanceof CompItem)) {
        throw new Error('HERO_LINE non precomp');
      }
    ...
    log('Nessuna heroline nel job, skip');
  }
  ...
  try {
    ...
    $.sleep(100);
    app.quit();
    log('✅ After Effects chiuso senza salvare');
  } catch(quitErr) {
    log('⚠️ Errore chiusura: ' + quitErr.toString());
  }
